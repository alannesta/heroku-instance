...
var Parser = {
	spawnPhantomProcess: (pageUrl) => {
		var phantomProcess = spawn('phantomjs', [path.join(process.cwd(), 'node_modules/91video-resolver/lib/resolver.js'), pageUrl]);
	...
	resolveVideoUrl: (phantomProcess, options, callback) => {
		...
		phantomProcess.on('exit', (code, signal) => {
			...
			if (code !== 0 && signal !== 'SIGKILL') {
				callback(new Error(status || 'UNHANDLED_EXIT'));
			}
		});
